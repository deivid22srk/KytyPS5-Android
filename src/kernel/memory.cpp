...
public:
...
#ifdef __ANDROID__
        /* Android: the full 13.5 GiB PS5 direct-memory backing needs one
         * contiguous writable mapping; on 39-bit-VA devices (512 GiB total,
         * already fragmented by the app, box64 dynarec and loaded game
        ...
        static uint64_t TotalSize() {
                static const uint64_t size = [] () -> uint64_t {
                        ...
                        uint64_t     mb = (env != nullptr) ? std::strtoull(env, nullptr, 10) : 4096;
                        if (mb < 256) mb = 256;
