...
#include "common/atomic_ref_compat.h"
...
/* bionic only defines PTHREAD_PRIO_NONE/PTHREAD_PRIO_INHERIT (priority
 * protection is not implemented there); keep the POSIX numeric value for
 * the attribute bookkeeping — the host mutexattr call is never made with it
 * (see SetMutexProtocol, which comments out setprotocol). */
#define PTHREAD_PRIO_PROTECT 2
#endif

#if defined(__ANDROID__) && !defined(__GLIBC__)
/* bionic removed thread cancellation entirely: no pthread_cancel,
 * pthread_setcancelstate/type, pthread_testcancel, and no PTHREAD_CANCEL_*
 * constants. Keep the kernel API working on Android: cancel state/type
 * round-trip through thread-local bookkeeping (matching the POSIX
 * defaults), testcancel is a no-op cancellation point, and an actual
 * cancellation request reports EOPNOTSUPP — host threads are never
 * aborted mid-execution (bionic's own stance on cancellation safety). */
#define PTHREAD_CANCEL_ENABLE 0
#define PTHREAD_CANCEL_DISABLE 1
#define PTHREAD_CANCEL_DEFERRED 0
#define PTHREAD_CANCEL_ASYNCHRONOUS 2

inline int pthread_setcancelstate(int state, int* old_state) {
        thread_local int g_cancel_state = PTHREAD_CANCEL_ENABLE;
        if (old_state != nullptr) {
                *old_state = g_cancel_state;
        }
        g_cancel_state = state;
        return 0;
}

inline int pthread_setcanceltype(int type, int* old_type) {
        thread_local int g_cancel_type = PTHREAD_CANCEL_DEFERRED;
        if (old_type != nullptr) {
                *old_type = g_cancel_type;
        }
        g_cancel_type = type;
        return 0;
}
...
inline int pthread_cancel(pthread_t) {
        return EOPNOTSUPP; // bionic hosts: cancellation is not supported
}
#endif
...
void* PthreadStaticObjects::CreateObject(void* addr, PthreadStaticObject::Type type) {
        ...
        auto       current_ref   = Common::atomic_ref<void*>(*current);
...
static int PthreadMutexInitNamed(PthreadMutex* mutex, const PthreadMutexattr* attr,
        ...
        Common::atomic_ref<PthreadMutexPrivate*>(*mutex).store(new_mutex, std::memory_order_release);
...
static int PthreadRwlockInitNamed(PthreadRwlock* rwlock, const PthreadRwlockattr* attr,
        ...
        Common::atomic_ref<PthreadRwlockPrivate*>(*rwlock).store(new_rwlock, std::memory_order_release);
...
static int PthreadCondInitNamed(PthreadCond* cond, const PthreadCondattr* attr, const char* name) {
        ...
        Common::atomic_ref<PthreadCondPrivate*>(*cond).store(new_cond, std::memory_order_release);
...
int KYTY_SYSV_ABI PthreadCancel(Pthread thread) {
        ...
        switch (result) {
                case 0: return OK;
                case ESRCH: return KERNEL_ERROR_ESRCH;
                case EOPNOTSUPP: return KERNEL_ERROR_EOPNOTSUPP; /* bionic hosts: unsupported */
                default: return KERNEL_ERROR_EINVAL;
        }
}
